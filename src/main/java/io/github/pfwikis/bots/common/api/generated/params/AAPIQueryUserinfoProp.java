...
public enum AAPIQueryUserinfoProp {
	...
	public static class Builder {
		private EnumSet<AAPIQueryUserinfoProp> set = EnumSet.noneOf(AAPIQueryUserinfoProp.class);

		public AAPIQueryUserinfoProp[] build() {
			return set.toArray(AAPIQueryUserinfoProp[]::new);
		}

		/**Echoes the <code>Accept-Language</code> header sent by the client in a structured format.*/
		public Builder ACCEPTLANG() {
			set.add(ACCEPTLANG);
			return this;
		}
		...
		/**Indicates whether the user is allowed to create accounts. To check whether some specific account can be created, use <a href="/wiki/Special:ApiHelp/query%2Busers" title="Special:ApiHelp/query+users"><kbd>action=query&amp;list=users&amp;usprop=cancreate</kbd></a>.*/
		public Builder CANCREATEACCOUNT() {
			set.add(CANCREATEACCOUNT);
			return this;
		}
		...
		/**Lists the groups the current user can add to and remove from.*/
		public Builder CHANGEABLEGROUPS() {
			set.add(CHANGEABLEGROUPS);
			return this;
		}
		...
		/**Adds the date of user's latest contribution.*/
		public Builder LATESTCONTRIB() {
			set.add(LATESTCONTRIB);
			return this;
		}
		...
		/**Lists all rate limits applying to the current user.*/
		public Builder RATELIMITS() {
			set.add(RATELIMITS);
			return this;
		}

		/**Adds the user's real name.*/
		public Builder REALNAME() {
			set.add(REALNAME);
			return this;
		}

		/**Adds the user's registration date.*/
		public Builder REGISTRATIONDATE() {
			set.add(REGISTRATIONDATE);
			return this;
		}
		...
		/**Lists all rate limits that would apply to the current user if they were not exempt from all ratelimits based on user rights or ip*/
		public Builder THEORETICALRATELIMITS() {
			set.add(THEORETICALRATELIMITS);
			return this;
		}

		/**Adds the count of unread pages on the user's watchlist (maximum 999; returns <samp>1000+</samp> if more).*/
		public Builder UNREADCOUNT() {
			set.add(UNREADCOUNT);
			return this;
		}
	}
